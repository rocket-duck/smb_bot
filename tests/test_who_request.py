...
@pytest.mark.asyncio
async def test_who_request_sends_bestoloch_on_force(tmp_path, monkeypatch):
    img_dir = tmp_path / "img"
    img_dir.mkdir()
    (img_dir / "bestoloch.jpg").write_bytes(b"fake")
    ...
    await who_request.handle_who_request(
        message, who_request_enable=True, force_bestoloch_enable=True
    )

    message.answer_photo.assert_awaited_once()
    photo = message.answer_photo.await_args.kwargs["photo"]
    assert str(photo.path).endswith("bestoloch.jpg")
