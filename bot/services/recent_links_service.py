...
class RecentLinksService:
    """
    Service for tracking recently sent links per chat
    and filtering them out based on TTL.
    """
    ...
    def filter(self, chat_id: int,
        ...
        for name, url in results:
            ...
            if last_time and (now - last_time) < timedelta(
                    minutes=self.ttl_minutes):
                logger.debug(f"Skipping recently sent link '{url}' "
                             f"for chat {chat_id}.")
            else:
